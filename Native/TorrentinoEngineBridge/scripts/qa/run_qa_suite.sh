...
#
# Role:     runs EVERY test_wp01_*.sh, test_wp02_*.sh, test_wp03_*.sh and
#           test_wp04_*.sh script (monotonic regression) and reports a
#           per-script pass/fail table. Exit 0 only if all scripts pass.
#
...
# Collect scripts in a stable (sorted) order: wp01 → wp02 → wp03 → wp04.
scripts=()
...
[[ ${#scripts[@]} -gt 0 ]] || { echo "no test_wp0{1,2,3,4}_*.sh scripts found" >&2; exit 2; }
...
wp01_n=0; wp02_n=0; wp03_n=0; wp04_n=0
for n in "${names[@]}"; do
	case "${n}" in
...
echo "total: ${#names[@]}  pass: ${pass_n}  fail: ${fail_n}  (wp01: ${wp01_n}  wp02: ${wp02_n}  wp03: ${wp03_n}  wp04: ${wp04_n})"
