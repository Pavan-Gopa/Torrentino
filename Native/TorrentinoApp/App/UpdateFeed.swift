...
///
/// The release owner publishes `appcast.xml` as a GitHub Release asset at
/// `releases/latest/download` and signs each release archive with Sparkle's
/// `sign_update` using the EdDSA private key paired with `SUPublicEDKey` in
/// Info.plist. The appcast and archive URLs must remain HTTPS-only. This
/// delegate-provided URL avoids a second, hardcoded `SUFeedURL` in Info.plist.
enum UpdateFeed {
